...
fn bench_convert(b: &mut Criterion) {
    ...
    b.bench_function("cl100k: convert tiktoken", |b| {
        b.iter(|| {
            Kitoken::from_tiktoken_slice(black_box(&data)).unwrap();
        })
    });
}
...
fn bench_encode_pride_and_prejudice(b: &mut Criterion) {
    ...
    let mut g = b.benchmark_group("cl100k: encode pride_and_prejudice");
    g.sampling_mode(criterion::SamplingMode::Flat);
    g.bench_function("full", |b| {
        let tokenizer = Kitoken::from_tiktoken_file(bench_models_path().join(MODEL_PATH)).unwrap();
        b.iter(|| {
            for _ in 0..10 {
...
fn bench_encode_utf8_sequence_0x10ffff(b: &mut Criterion) {
    ...
    let mut g = b.benchmark_group("cl100k: encode utf8_sequence_0x10ffff");
    g.sampling_mode(criterion::SamplingMode::Flat);
    g.bench_function("full", |b| {
        let tokenizer = Kitoken::from_tiktoken_file(bench_models_path().join(MODEL_PATH)).unwrap();
        b.iter(|| {
...
fn bench_encode_wagahai(b: &mut Criterion) {
    ...
    let mut g = b.benchmark_group("cl100k: encode wagahai");
    g.sampling_mode(criterion::SamplingMode::Flat);
    g.bench_function("full", |b| {
        let tokenizer = Kitoken::from_tiktoken_file(bench_models_path().join(MODEL_PATH)).unwrap();
        b.iter(|| {
