//! Test for the conversion of the huggingface llama2 bpe tokenizer definition.
...
fn init_kitoken() -> &'static Kitoken {
    ...
    TOKENIZER.get_or_init(|| {
        let data = std::fs::read(test_models_path().join("llama2.json")).unwrap();
        let definition = convert_huggingface(data).unwrap();
...
fn test_serialize_deserialize() {
    let data = std::fs::read(test_models_path().join("llama2.json")).unwrap();
    let definition1 = convert_huggingface(data).unwrap();
...
fn test_small_lines() {
    ...
    test_encode_decode_lines_same(tokenizer, "small_input.txt", "small_tokens_llama2.txt", false);
}

#[test]
fn test_utf8_full() {
    let tokenizer = init_kitoken();
    test_encode_decode_full_same(tokenizer, "utf8_input.txt", "utf8_tokens_llama2.txt");
}
