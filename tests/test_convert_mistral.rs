//! Test for the conversion of the sentencepiece mistral bpe model.
...
fn init_kitoken() -> &'static Kitoken {
    ...
    TOKENIZER.get_or_init(|| {
        let data = std::fs::read(test_models_path().join("mistral.model")).unwrap();
...
fn test_small_lines() {
    ...
    test_encode_decode_lines_same(tokenizer, "small_input.txt", "small_tokens_mistral.txt", true);
}

#[test]
fn test_utf8_full() {
    let tokenizer = init_kitoken();
    test_encode_decode_full_same(tokenizer, "utf8_input.txt", "utf8_tokens_mistral.txt");
}
