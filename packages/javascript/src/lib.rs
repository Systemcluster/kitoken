...
impl Kitoken {
    ...
    pub fn new(data: &[u8]) -> Result<Kitoken, JsValue> {
        Ok(Kitoken {
            inner: Rc::new(
                Inner::from_slice(data).map_err(|e| JsValue::from_str(&format!("{:?}", e)))?,
            ),
        })
    }
    ...
    pub fn encode(&self, text: &str, encode_specials: Option<bool>) -> Result<Vec<u32>, JsValue> {
        ...
        //     .collect::<Vec<u16>>();
        // let text = String::from_utf16(&text).map_err(|e| JsValue::from_str(&format!("{:?}", e)))?;
        self.inner
    ...
    pub fn from_sentencepiece(data: &[u8]) -> Result<Kitoken, JsValue> {
        Ok(Kitoken {
            inner: Rc::new(
                Inner::from_sentencepiece_slice(data)
                    .map_err(|e| JsValue::from_str(&format!("{:?}", e)))?,
            ),
        })
    }
    ...
    pub fn from_tiktoken(data: &[u8]) -> Result<Kitoken, JsValue> {
        Ok(Kitoken {
            inner: Rc::new(
                Inner::from_tiktoken_slice(data)
                    .map_err(|e| JsValue::from_str(&format!("{:?}", e)))?,
            ),
        })
    }
    ...
    pub fn from_tokenizers(data: &[u8]) -> Result<Kitoken, JsValue> {
        Ok(Kitoken {
            inner: Rc::new(
                Inner::from_tokenizers_slice(data)
                    .map_err(|e| JsValue::from_str(&format!("{:?}", e)))?,
            ),
        })
    }
